...
const CollectionAccountPayroll = mongoose.model("CollectionAccountPayroll", collectionAccountPayrollSchema);
...
app.delete("/collection-accounts/:id/payments/:paymentId", auth, allow("ADMIN"), async (req, res) => {
  try {
...
app.get("/collection-account-payrolls", auth, allow("ADMIN"), async (req, res) => {
  try {
    const filter = {};
...
app.get("/collection-account-payrolls/:id", auth, allow("ADMIN"), async (req, res) => {
  try {
...
app.get("/collection-accounts/:id/payroll-data", auth, allow("ADMIN"), async (req, res) => {
  try {
    const { id } = req.params;
...
app.post("/collection-account-payrolls", auth, allow("ADMIN"), async (req, res) => {
  try {
    const {
      collectionAccountId,
      planillaNumber,
...
app.delete("/collection-account-payrolls/:id", auth, allow("ADMIN"), async (req, res) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return res.status(400).json({ error: "ID de planilla inválido" });
    }
